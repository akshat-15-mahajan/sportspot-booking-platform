...
export default function VenueAuth() {
    ...
    try {
      if (mode === 'login') {
        ...
        if (result.status === 'complete') {
          ...
          // Don't set loading=false — let Clerk state propagation
          // trigger AuthProvider's useEffect for profile fetch.
        ...
        if (!form.fullName || !form.phone) throw new Error('Please fill in all fields')
        await signUp.create({
          emailAddress: form.email,
          password: form.password,
          firstName: form.fullName.split(' ')[0],
          lastName: form.fullName.split(' ').slice(1).join(' ') || '',
          unsafeMetadata: { role: 'venue', phone: form.phone, city: form.city },
        })
  ...
  return (
    <div className="auth-screen" style={{ background: '#050510' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 60% 50% at 30% 40%, rgba(77,159,255,0.08) 0%, transparent 60%)', pointerEvents: 'none' }} />
      ...
      <div className="auth-hero">
        ...
        <div className="auth-hero-title">
          List your<br />
          <span style={{ color: 'var(--blue)' }}>sports</span><br />
          facility.
        </div>
        <p className="auth-hero-sub">
          Reach more players, manage bookings easily, and grow your revenue.
        </p>
        <div className="auth-hero-sports">
          {[['📅','Bookings'],['💰','Revenue'],['📊','Analytics'],['🏟️','Venues']].map(([icon, label]) => (
      ...
      <div className="auth-card" style={{ border: '1px solid rgba(77,159,255,0.2)' }}>
        <Logo height={36} style={{ marginBottom: "1.25rem" }} />

        <div className="auth-portal-badge" style={{ color: 'var(--blue)', borderColor: 'var(--blue-bg)', background: 'var(--blue-bg)' }}>
          🏢 Venue Owner Portal
        </div>

        <h1 className="auth-title">
          {needsProfileCompletion ? 'Complete Your Profile' : pendingSecondFactor ? 'Verify Sign-In' : pendingVerification ? 'Verify Email' : mode === 'login' ? 'Venue Sign In' : 'Register Your Venue'}
        </h1>
        <p className="auth-subtitle">
          {needsProfileCompletion ? 'Just a few more details to list your venue' : pendingSecondFactor ? secondFactorHint : pendingVerification ? 'Enter the code sent to your email' : mode === 'login' ? 'Manage your listings and bookings' : 'List your sports facility on Rabonna'}
        </p>
        ...
        {needsProfileCompletion ? (
          <form onSubmit={async (e) => {
            e.preventDefault()
            if (!profileForm.fullName || !profileForm.phone) { setError('Please fill in all required fields'); return }
            setLoading(true); setError('')
            try {
              await createProfile({ fullName: profileForm.fullName, phone: profileForm.phone, city: profileForm.city, role: 'venue' })
            } catch (err) { setError(err.message || 'Failed to create profile') }
            finally { setLoading(false) }
          }}>
            <div className="form-group">
              <label className="form-label">Business / Your Name</label>
              <input className="form-input" placeholder="XYZ Sports Centre" value={profileForm.fullName}
            ...
            <div className="form-group">
              ...
              <input className="form-input" placeholder="New Delhi"
                value={profileForm.city} onChange={e => updateProfile('city', e.target.value)} />
            </div>
            {error && <div className="auth-error">{error}</div>}
            <button type="submit" className="btn btn-full btn-lg" disabled={loading}
              style={{ background: 'var(--blue)', color: 'white' }}>
              {loading && <span className="inline-spinner" style={{ borderTopColor: 'white' }} />}
              {loading ? 'Saving…' : 'Continue →'}
            </button>
            <div style={{ marginTop: '1rem', padding: '0.85rem 1rem', background: 'var(--amber-bg)', borderRadius: 10, border: '1px solid var(--amber)', fontSize: '0.78rem', color: 'var(--amber)', lineHeight: 1.5 }}>
              ⚠️ New venue accounts require admin approval before listings go live.
            </div>
          </form>
        ) : isSignedIn && !pendingVerification && !profileError ? (
          <div style={{ textAlign: 'center', padding: '2rem 0' }}>
            <span className="inline-spinner" style={{ width: 28, height: 28, borderTopColor: 'var(--blue)' }} />
          ...
          <div style={{ textAlign: 'center', padding: '1.5rem 0' }}>
            ...
            <p style={{ color: 'var(--text-3)', fontSize: '0.85rem', margin: '0 0 1rem' }}>
              We could not load your venue profile on this device.
            </p>
            <div style={{ display: 'flex', gap: '0.6rem' }}>
              <button type="button" className="btn btn-full" style={{ background: 'var(--blue)', color: 'white' }} onClick={() => refreshProfile().catch(() => {})}>
                Retry
              </button>
          ...
          <>
            ...
            <form onSubmit={handleSubmit}>
              {mode === 'signup' && (
                <div className="form-group">
                  <label className="form-label">Business / Your Name</label>
                  <input className="form-input" placeholder="XYZ Sports Centre" value={form.fullName}
            ...
            <div className="auth-switch">
              {mode === 'login' ? 'New venue? ' : 'Already registered? '}
              <button className="auth-switch-btn" style={{ color: 'var(--blue)' }}
